...
document.addEventListener('DOMContentLoaded', () => {
    // Tu lista de nombres (puede venir de una API)
    const empleados = [
        "Angel Hernandez Sánchez",
        "Ana María López García",
        "Arturo Pérez Rodríguez",
        "Brenda Martínez Cruz",
        "Carlos Alberto Ruiz Jiménez",
        "Claudia Ochoa Vázquez"
    ];

    const inputEmpleado = document.getElementById('nombre-empleado');
    const listaCoincidencias = document.getElementById('coincidencias-lista');

    // Escucha cada vez que el usuario escribe
    inputEmpleado.addEventListener('input', () => {
        const textoBusqueda = inputEmpleado.value.toLowerCase();

        // Limpiar resultados anteriores
        listaCoincidencias.innerHTML = '';

        if (textoBusqueda.length === 0) {
            // Oculta el menú si no hay texto
            listaCoincidencias.classList.remove('show');
            return;
        }

        const coincidencias = empleados.filter(nombre =>
            nombre.toLowerCase().includes(textoBusqueda)
        );

        if (coincidencias.length > 0) {
            // Muestra el menú si hay coincidencias
            listaCoincidencias.classList.add('show');
            ...
            coincidencias.forEach(nombre => {
                ...
                // Al hacer clic, se actualiza el input y se oculta el menú
                item.addEventListener('click', (e) => {
                    e.preventDefault();
                    inputEmpleado.value = nombre;
                    listaCoincidencias.classList.remove('show');
                });
            ...
            // Oculta el menú si no hay coincidencias
            listaCoincidencias.classList.remove('show');
        }
    });

    // Oculta la lista si el usuario hace clic en cualquier otro lugar de la página
    document.addEventListener('click', (event) => {
        if (!inputEmpleado.contains(event.target)) {
            listaCoincidencias.classList.remove('show');
        }
    });
});
